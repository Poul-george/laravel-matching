...
<html>
    ...
    <?php
      $search_word = session()->get('user_search_word');
    ?>
        ...
        <div class="main_div" id="main_div">
            ...
            @endif
              ...
              @if (count($user_item) !== 0)
                @foreach ($user_item as $one_item)
                <div class="administrator_account_info_div">
                  <div class="administrator_account_one_div administrator_id">
                    <a class="black_href" href="{{ asset(config('const.title.title49'))}}/user_account_detail/{{$one_item->id}}">
                      <p class="administrator_account_info_p">{{$one_item->user_id}}</p>
                    </a>
                  </div>
                  
                  <div class="administrator_account_one_div administrator_name">
                    <p class="administrator_account_info_p">{{$one_item->user_name}}</p>
                  </div>
                  
                  <div class="administrator_account_one_div administrator_phone">
                    <p class="administrator_account_info_p">{{$one_item->user_phone}}</p>
                  </div>
            ...
            <div class="pagination_div">
                <div class="pagination_ul_div">
                    <ul class="pagination">
                        ...
                        @else
                            <li class="pagination_li not_disabled"><a rel="prev" href="{{ asset(config('const.title.title49'))}}//user_account?page={{$prev}}" class="btn prev_next_btn">前のページ</a></li>
                        @endif

                        @if ($next !== 0)
                            <li class="pagination_li not_disabled"><a rel="next" href="{{ asset(config('const.title.title49'))}}//user_account?page={{$next}}" class="btn prev_next_btn">次のページ</a></li>
                        @else
